...
---
```sql
	--- caso haja uma conversão onde a tabela tenha 3 chaves 2 repitidas e 1 não é necessario passar as 3 chaves no join como abaixo. 
SELECT
	DISTINCT C.CodRedCt ID_VINCULO,
	3 TIPO,
	CAST(M.CodIBGEMunic AS INTEGER) ID_CIDADE,
	0 ID_ESTADO,
	C.EndFatCli LOGRADOURO,
	C.NrEndFatCli NUMERO,
	C.ComplFatCli COMPLEMENTO,
	L.NomeLoc 
FROM
	TbCli C
LEFT JOIN TbMunic M ON
	M.CodMunic = C.CodMunicFat
	AND M.UFMunic = SUBSTRING(C.UFMunicFat, 1, 2)
LEFT JOIN TBloc L ON l.CodLoc  = C.CodLocFat  AND L.CodMunic = M.CodMunic AND L.UFMunic = M.UFMunic 
``` 
## FUNÇÕES sql QUE PODEM AGREGAR na conversão ## 
...
--VE A LIGAÇÃO ENTRE 2 TABELA FIREBIRD
SELECT 
    a.RDB$FIELD_NAME AS COLUNA_COMUM,
    a.RDB$RELATION_NAME AS TABELA1,
    b.RDB$RELATION_NAME AS TABELA2
FROM RDB$RELATION_FIELDS a
JOIN RDB$RELATION_FIELDS b
    ON a.RDB$FIELD_NAME = b.RDB$FIELD_NAME
WHERE a.RDB$RELATION_NAME = 'PESSOAS'
  AND b.RDB$RELATION_NAME = 'ENDERECOS'
  AND a.RDB$RELATION_NAME <> b.RDB$RELATION_NAME;
...
WHERE TRIM(f.RDB$FIELD_NAME) = 'ID_CIDADE'
  AND r.RDB$SYSTEM_FLAG = 0   -- ignora tabelas do sistema
